...
아래 두 개는 Figma `Scale` 컬렉션에 없는 값으로, 과제 진행 중 추가했습니다.
기존 토큰 값은 하나도 수정하지 않았습니다.

| Dart 필드 | 값 | 추가한 이유 |
|---|---|---|
| `iconLg` | 22 | 시안의 탭 바 아이콘, 검색 결과 행의 관심 별, 상세 헤더의 관심 별이 모두 22다. `iconSm`(16) / `iconMd`(20) 어느 쪽에도 해당하지 않아 화면마다 22를 직접 쓰게 되어 토큰으로 올렸다. |
| `fieldHeight` | 40 | 검색 입력창의 높이가 시안에서 40으로 고정되어 있다. `rowMinHeight`(56) / `tabBarHeight`(56)와 같은 성격의 "컴포넌트 고정 높이" 토큰이라 같은 자리에 두었다. |
